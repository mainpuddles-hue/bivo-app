...
describe('Email Format Validation', () => {
  ...
  test('Standard email addresses are valid', () => {
    ...
    expect(isValidEmail('jesse@bivoapp.io')).toBe(true)
...
describe('Profile Creation Fallback', () => {
  ...
  test('Profile insert has correct fields', () => {
    const insert = buildProfileInsert('user-123', 'jesse@bivoapp.io', 'Jesse')
    expect(insert).toEqual({
      id: 'user-123',
      email: 'jesse@bivoapp.io',
      name: 'Jesse',
    })
  })

  test('Profile insert trims email and name', () => {
    const insert = buildProfileInsert('user-123', '  jesse@bivoapp.io  ', '  Jesse Parkkonen  ')
    expect(insert.email).toBe('jesse@bivoapp.io')
